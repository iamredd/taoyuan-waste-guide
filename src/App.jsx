  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24">
        ...
        <div className="max-w-2xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-100 uppercase tracking-widest font-medium">垃圾分類查詢系統</p>
            </div>
          </div>
      ...
      <main className="max-w-2xl mx-auto px-6 pt-8 space-y-8">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl shadow-emerald-900/5 border border-emerald-50 animate-slide-up">
          ...
          <div className="flex flex-wrap gap-2 justify-center mt-6">
            {['廚餘', '紙容器', '電池', '廢紙'].map(tag => (
              <button
                key={tag}
                onClick={() => setSearchTerm(tag)}
                className="px-4 py-1.5 bg-emerald-50 text-emerald-700 rounded-full text-xs font-bold hover:bg-emerald-100 transition-all border border-emerald-100 shadow-sm"
              >
                {tag}
              </button>
            ))}
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="font-bold text-slate-500 text-xs uppercase tracking-widest">
              {searchTerm ? '查詢結果' : '快速建議'}
            </h3>
            {searchTerm && (
              <button onClick={() => setSearchTerm('')} className="text-xs font-bold text-rose-500 flex items-center gap-1">
          ...
          <div className="grid gap-4">
            {filteredItems.length > 0 ? (
              filteredItems.map(item => (
                <div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex gap-5 items-start animate-slide-up">
              ...
              <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-slate-200">
                ...
                <p className="text-slate-500 font-medium">找不到相關項目，建議依照「一般垃圾」處理<br />或詢問社區管理員。</p>
              </div>
            )}
          </div>
        </div>

        {/* Official Links Update */}
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
          <h3 className="font-bold text-slate-800 mb-6">更詳細的官方指南</h3>
          <div className="grid grid-cols-1 gap-3">
            <a href="https://guide.mote.gov.tw/" target="_blank" rel="noreferrer" className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-emerald-50 transition-colors">
              <span className="font-bold text-sm text-slate-700 flex items-center gap-3">
                <Recycle className="w-5 h-5 text-emerald-600" /> 環境部：回收大百科
              </span>
              <ExternalLink className="w-4 h-4 text-slate-300" />
            </a>
            <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
              <p className="text-[10px] font-bold text-amber-800 uppercase mb-1">提示</p>
              <p className="text-xs text-amber-900 leading-relaxed">桃園區公告：廢玻璃瓶、廢乾電池、廢照明光源、廢塑膠袋等資源物，請務必分開交給回收車，不可混入垃圾中。</p>
            </div>
          </div>
        </div>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 px-10 py-4 flex justify-between items-center z-50">
        <button onClick={() => { setSearchTerm(''); setActiveTab('home'); }} className={`flex flex-col items-center gap-1 ${!searchTerm ? 'text-emerald-600' : 'text-slate-400'}`}>
        ...
        <button onClick={() => setActiveTab('guide')} className={`flex flex-col items-center gap-1 ${activeTab === 'guide' ? 'text-emerald-600' : 'text-slate-400'}`}>
